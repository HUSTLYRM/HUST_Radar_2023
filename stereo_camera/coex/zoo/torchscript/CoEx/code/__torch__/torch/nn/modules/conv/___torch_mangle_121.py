class ConvTranspose3d(Module):
  ...
  def forward(self: __torch__.torch.nn.modules.conv.___torch_mangle_121.ConvTranspose3d,
    input: Tensor,
    output_size: Optional[List[int]]=None) -> Tensor:
    output_padding = (self)._output_padding(input, output_size, [2, 2, 2], [1, 1, 1], [4, 4, 4], [1, 1, 1], )
    _0 = torch.conv_transpose3d(input, self.weight, self.bias, [2, 2, 2], [1, 1, 1], output_padding, 1, [1, 1, 1])
    return _0
  def _output_padding(self: __torch__.torch.nn.modules.conv.___torch_mangle_121.ConvTranspose3d,
    input: Tensor,
